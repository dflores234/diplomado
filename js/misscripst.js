...
function listaRecurso(index) {
    $.ajax({
        type: "POST",
        url: "php/listarRecurso.php",
        dataType: "JSON",
        success: function(response) {
            $.each(response, function(indice, valor) {
                if (valor.id_modulo == index) {
                    $('#card-mod-recurso').html('');
                    $('#card-mod-recurso').append('<h4>' + valor.nombre_recurso + '</h4><br><p>' + valor.descripcion_recurso + '</p><a href="' + valor.link_recurso + '">Link del artículo</a><hr class="style-two" />');
                }
            });
        }
    });
